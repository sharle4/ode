...
export const getTrendingPoems = (limit: number = 10) => executeCachedQuery(
    {
        keyParts: [`trending-poems-v2-${limit}`],
...
export const getCollectionWithSections = (slug: string) => executeCachedQuery(
    ...
    async (supabase) => {
        ...
        const { data: collection } = await supabase
            .from('collections')
            .select('id, title, slug, publication_year, summary, cover_url, poems_count, average_review, reviews_count, likes_count, authors ( id, name, slug )')
            .eq(isId ? 'id' : 'slug', slug)
            .maybeSingle()
            .throwOnError();
...
export const getUserProfileByUsername = (username: string) => executeCachedQuery(
    {
        keyParts: ['user-profile', username.toLowerCase()],
        tags: [CACHE_TAGS.profile(username)],
        revalidate: 60, // 60 seconds caching with background revalidation
    ...
    async (supabase) => {
        ...
        // 2. Fetch all child data concurrently in a single parallel batch
        const [
            [readsResult, reviewsResult, listsResult, followersResult, followingResult],
            topPoemLinksResult,
            topAuthorLinksResult,
            recentReviewsResult,
            userBadgesResult,
            allReviewsResult,
            ...
            supabase
                ...
                .eq('user_id', user.id)
                .throwOnError(),
            // All reviews for score distribution
            supabase
                .from('poem_reviews')
                .select('score')
                .eq('user_id', user.id)
        ...
        const topPoems = (topPoemLinksResult.data || [])
            .map((link: any) => {
        ...
        const topAuthors = (topAuthorLinksResult.data || [])
            .map((link: any) => ({ ...link.authors, position: link.position }))
            .filter((a: any) => a.id);

        const recentReviews = recentReviewsResult.data || [];

        const badges = (userBadgesResult.data || [])
            .map((link: any) => ({ ...link.badges, unlocked_at: link.unlocked_at }))
            .filter((b: any) => b.id);
...
export const getUserLikesByUsername = (username: string) => executeCachedQuery(
    ...
    async (supabase) => {
        ...
        const likedPoems = (likedPoemsRes.data || [])
            .map((item: any) => {
                const poem = item.poems;
                if (!poem) return null;
                return {
                    ...poem,
                    liked_at: item.created_at,
                    authors: (poem.authors || []).map((a: any) => a.authors).filter(Boolean),
                    rothko_params: Array.isArray(poem.rothko_params) ? poem.rothko_params[0] : poem.rothko_params,
                };
            })
            .filter(Boolean);

        const likedCollections = (likedCollectionsRes.data || [])
            .map((item: any) => {
        ...
        const likedAuthors = (likedAuthorsRes.data || [])
            .map((item: any) => {
