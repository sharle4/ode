...
interface PoemData { id: number; title: string; authors: { name: string | null } | null; publication_date: string | null; source: string | null; categories: string[] | null; }
...
export default function PoemInfoCard({ poem, stats, distribution, user, userLists, userRating, onRate, onCritique, onAddToList }: PoemInfoCardProps) {
  ...
  const isInteractive = !!onRate && !!onCritique && !!onAddToList;
    ...
    <div className="sticky top-24">
      <div className="bg-white dark:bg-gray-800/50 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-800">
        <Link href={`/poemes/${poem.id}`}>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white truncate hover:text-indigo-600 dark:hover:text-indigo-400">
            {poem.title}
          </h2>
        </Link>
        ...
        {isInteractive && (
          <>
            ...
            <div className="mt-4">
              <Rating rating={userRating || null} setRating={onRate} hoverRating={hoverRating} setHoverRating={setHoverRating} />
            </div>
