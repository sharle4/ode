import Link from 'next/link'
import StaticRating from './StaticRating'

interface Review {
  id: number
  rating: number | null
  content: string | null
  poems: {
    id: number
    title: string | null
    authors: {
      name: string | null
    } | null
  } | null
}

interface ReviewCardProps {
  review: Review
}

export default function ReviewCard({ review }: ReviewCardProps) {
  if (!review.poems || !review.poems.authors) {
    return null
  }

  const CritiqueContent = () => (
    <>
      <div className="mt-2">
        <StaticRating rating={review.rating} size="md" />
      </div>
      {review.content && (
        <p className="mt-3 text-gray-600 dark:text-gray-400 text-sm italic border-l-2 border-gray-200 dark:border-gray-700 pl-3">
          "{review.content}"
        </p>
      )}
    </>
  )

  return (
    <div className="bg-white dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 p-4 flex flex-col h-full">
      <div className="flex-grow">
        <Link href={`/poemes/${review.poems.id}`}>
          <h3 className="font-bold text-lg text-gray-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
            {review.poems.title}
          </h3>
        </Link>
        <Link href={`/auteurs/${encodeURIComponent(review.poems.authors.name || '')}`}>
          <p className="text-sm text-gray-500 dark:text-gray-400 hover:underline">
            {review.poems.authors.name || 'Auteur inconnu'}
          </p>
        </Link>
        
        {review.content ? (
          <Link href={`/critiques/${review.id}`}>
            <CritiqueContent />
          </Link>
        ) : (
          <CritiqueContent />
        )}
      </div>
    </div>
  )
}